...
{
    public class SimpleForFiveMinutesAndParabolicSar : IExternalScript
    {
        ...
        public static OptimProperty AccelerationMax = new OptimProperty(0.02, 0.01, 1, 0.01);
        public static OptimProperty AccelerationStart = new OptimProperty(0.02, 0.01, 1, 0.01);
        public static OptimProperty AccelerationStep = new OptimProperty(0.02, 0.01, 1, 0.01);
        ...
        public virtual void Execute(IContext ctx, ISecurity source)
        {
            ...
            {
                Trading(ctx, source, compressSource, historyBar, buySignal, sellSignal, parabolic);
            }
            ...
            IList<double> parabolic)
        {
            ...
            if (source.Positions.ActivePositionCount > 0)
            {
                SearchActivePosition(source, actualBar, parabolic);
            }
        ...
        public void SearchActivePosition(ISecurity source, int actualBar, IList<double> parabolic)
        {
            ...
            foreach (var position in positionList)
            {
                ...
                switch (arr[0])
                {
                    case "buy":
                        ...
                        position.CloseAtStop(actualBar + 1, Convert.ToDouble(parabolic[actualBar]), Slippage, "closeStop");
                        break;
                    case "sell":
                        position.CloseAtProfit(actualBar + 1, Convert.ToDouble(arr[1]) - ScopeProfite, "closeProfit");
                        position.CloseAtStop(actualBar + 1, Convert.ToDouble(parabolic[actualBar]), Slippage, "closeStop");
